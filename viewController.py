...
def observer_rcpereports_view(mac):
    ...
    end_date = datetime.datetime.now().replace(second=0)
    start_date = end_date - datetime.timedelta(days=1)
    ...
    # router_status = get_status(router_ip)
    # traffic_metric = models.MetricDefinition.objects.filter(name='CPE_RXTX').first()
    # coverage = reports_services.get_average_coverage(device, start_date, end_date)
    # traffic_average = reports_services.get_metric_numeric_average(device, traffic_metric, start_date, end_date)
    # capacity_average = reports_services.get_metric_numeric_average(device, metric, start_date, end_date)
    #
    # latlong = metrics_services.get_lastknown_latlong(device)
    ctx = {
        'chart_id': 'avg_qos',
        'chart_title': 'Capacity and Traffic for '+device.name,
        'chart_data': [
        ...
        'start_date': datetime.datetime.strftime(start_date, '%Y-%m-%d %H:%M:%S'),
        'end_date': datetime.datetime.strftime(end_date, '%Y-%m-%d %H:%M:%S'),
        'data_url': '/metrics_with_gps_trail',
    ...
    return render_template('capacity_high_reports.html', **ctx)

# @app.route('/reports/metrics_with_gps_trail', methods=['POST','GET'])
# def reports_gps_metrics_trail():
#     logger = app.logger
#
#     device_mac = flask.request.values.get('device_mac')
#     start_time = flask.request.values.get('start_time')
#     end_time = flask.request.values.get('end_time')
#
#     # Check request data
#     if not device_mac:
#         logger.error('reports_gps_trail: no device_mac parameter found in request.')
#         return flask.abort()
#
#     if not start_time:
#         logger.warn('reports_gps_trail: no start_time found in request.')
#         return flask.abort()
#
#     if not end_time:
#         logger.warn('reports_gps_trail: no end_time found in request.')
#         return flask.abort()
#
#     metrics_count = int(flask.request.values.get('metrics_count'))
#     metrics = []
#     for i in range(0, metrics_count):
#         n = "metric_%d" % i
#         metrics.append(flask.request.values.get(n))
#     metrics.append("CPE_GEO")
#     metrics.append("TPW_NOW")
#
#     metric_definitions = models.MetricDefinition.objects.filter(name__in=metrics)
#     request_device = models.ManagedDevice.objects.filter(mac=device_mac).first()
#
#     bts_device = None
#     cpe_device = None
#     if request_device.device_class.type == 'CPE':
#         cpe_device = request_device
#     else:
#         bts_device = request_device
#
#     start_time = int(start_time)
#     end_time = int(end_time)
#
#     heatmap = Heatmap()
#     heatmap.add_interval(100, 30, '#003300')
#     heatmap.add_interval(30, 20, '#006600')
#     heatmap.add_interval(20, 10, '#009933')
#     heatmap.add_interval(10, 5, '#33CC33')
#     heatmap.add_interval(5, 2, '#99FF33')
#     heatmap.add_interval(2, -1, '#ffa500')
#
#
#     sections = get_averages_and_gps_trail(bts_device, metric_definitions, start_time, end_time, cpe_device, heatmap=heatmap)
#
#     response_data = {}
#
#     for mdef in metric_definitions:
#         response_data[mdef.name] = []
#
#     for key, section in sections.items():
#         for mdef in metric_definitions:
#
#             if mdef.name == 'CPE_GEO':
#                 response_data[mdef.name].append([section['time'], section[mdef.name]['geo']])
#             else:
#                 response_data[mdef.name].append([section['time'], section[mdef.name]])
#             # if len(response_data[mdef.name]) == 0:
#             #     response_data[mdef.name].append([section['time'], section[mdef.name]])
#             # elif section[mdef.name] != 0:
#             #     response_data[mdef.name].append([section['time'], section[mdef.name]])
#             # elif response_data[mdef.name][-1][1] != section[mdef.name]:
#             #     response_data[mdef.name].append([section['time'], section[mdef.name]])
#
...
def get_averages_and_gps_trail(bts, metric_definitions, start_time, end_time, cpe=None, max_sections=180, heatmap=None):
    ...
    # group by time and metric definition
    records_dd = collections.defaultdict(lambda: dict())
    records = models.MetricRecord.objects.only(*only_fields).filter(q).select_related()  #todo
    for record in records:
        rtime = int(calendar.timegm(record.time.timetuple()))
        records_dd[rtime][record.metric_definition.name] = record
    ...
    for x in range(0, max_sections+1):
        ...
        for mdef in metric_definitions:  #todo
            sections[x][mdef.name] = {'sum': 0, 'count': 0, 'average': 0, 'geo': None,
                                      'freqs': collections.defaultdict(int)}
        ...
        for metric, record in metrics_dict.iteritems():
            ...
            if record.metric_definition.geo:
                ...
                if len(a) == 0:
                    ...
                    if prev_marker is not None:
                        distance = dataController.distance_in_mileslatlong(prev_marker['lat'], prev_marker['lng'],
                                                                    gps_marker['lat'], gps_marker['lng'])
                        gps_marker['distance'] = distance
                    ...
                    distance = dataController.distance_in_mileslatlong(a[-1]['lat'], a[-1]['lng'],
                                                                    gps_marker['lat'], gps_marker['lng'])
                    gps_marker['distance'] = distance
                    if distance < 0.5:
                        continue
...
class Heatmap():
    ...
    def add_interval(self, upper_value, lower_value, color_value):
        self.lower.append(lower_value)
        self.upper.append(upper_value)
        self.color.append(color_value)

    def get_color_for_value(self, value):

        for x, y, c in zip(self.upper, self.lower, self.color):
            if x > value >= y:
                return c
        return None


@app.route('/metrics_with_gps_trail', methods=['POST','GET'])
def reports_gps_metrics_trail():
    logger = app.logger
